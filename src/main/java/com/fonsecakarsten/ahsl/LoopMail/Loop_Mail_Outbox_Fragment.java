...
public class Loop_Mail_Outbox_Fragment extends ListFragment {

    private LoopMailAdapterOutbox adapter;

    private enum LoopMailBoxType {
        SENT
    }

    public Loop_Mail_Outbox_Fragment() {
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ScrapeLoopMailTaskOutbox task = new ScrapeLoopMailTaskOutbox();
        task.execute(LoopMailBoxType.SENT);
    }
    ...
    private class ScrapeLoopMailTaskOutbox extends AsyncTask<LoopMailBoxType, Void, List<MailEntry>> {
        @Override
        protected List<MailEntry> doInBackground(LoopMailBoxType... params) {
            try {
                return API.get().getMailOutbox();
            } catch (Exception e) {
        ...
        protected void onPostExecute(List<MailEntry> result) {
            ...
            if (values.length > 0) {
                ...
                adapter = new LoopMailAdapterOutbox(getActivity(), values);

                listView.setAdapter(adapter);

                listView.setOnItemClickListener(new MailItemClickAdapterOutbox(adapter, getActivity()));
    ...
    private class LoopMailAdapterOutbox extends ArrayAdapter<MailEntry> {
        private final Context context;
        private final MailEntry[] values;
        private boolean sortDir;

        public LoopMailAdapterOutbox(Context context, MailEntry[] values) {
    ...
    private class ScrapeMailContentTaskOutbox extends AsyncTask<MailEntry, Void, MailDetail> {
        private View flow;
        private View contwrap;
        private ProgressBar bar;

        public ScrapeMailContentTaskOutbox(View flow, View contwrap, ProgressBar bar) {
            this.flow = flow;
            this.bar = bar;
        ...
        protected MailDetail doInBackground(MailEntry... params) {
            try {
                return API.get().getMailDetailsOutbox(params[0]);
            } catch (Exception e) {
                RemoteDebug.debugException(e);
            }
    ...
    private class MailItemClickAdapterOutbox implements AdapterView.OnItemClickListener {
        LoopMailAdapterOutbox adapter;
        Activity parent;

        public MailItemClickAdapterOutbox(LoopMailAdapterOutbox adapter, Activity parent) {
        ...
        public void onItemClick(AdapterView<?> list, View view, int position, long id) {
            ...
            ScrapeMailContentTaskOutbox task = new ScrapeMailContentTaskOutbox(flow, contwrap, load);
